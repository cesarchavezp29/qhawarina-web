    ...
    <div className="relative max-w-5xl mx-auto px-4 sm:px-6 py-12 space-y-16" style={{ zIndex: 1 }}>
      ...
      <section className="space-y-3 pt-2">
        <p className="text-xs text-stone-400 font-medium tracking-wide">
          Salario Mínimo / Distribución
        </p>
        <h1 className="text-3xl sm:text-4xl font-black text-stone-900 leading-tight">
          La distribución salarial antes y después
        </h1>
        <p className="text-stone-500 max-w-2xl">
          Trabajadores formales dependientes · ENAHO · Cada barra muestra el cambio en la
          participación de trabajadores en ese tramo salarial (post − pre). Barras rojas:
          masa que desaparece bajo el nuevo SM. Barras verdes: masa que reaparece por encima.
        </p>
      </section>
      ...
      <FadeSection className="space-y-6">
        ...
        >
          ...
          <p className="text-xs text-stone-400">
            ...
            {ev.ci_hi >= 1
              ? 'No rechaza R=1 (compatible con redistribución perfecta)'
              : 'Rechaza R=1 al 95%'} ·
      ...
      <FadeSection className="space-y-4">
        <h2 className="text-xl font-bold text-stone-900">¿Cómo leer este gráfico?</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
              color: TERRACOTTA,
              icon: '↓',
              title: 'Zona roja — masa desaparecida',
              body: `Los trabajadores que ganaban entre S/${Math.round(0.85 * ev.mw_old)} y S/${ev.mw_new} en el período pre desaparecen de esa zona en el período post. El aumento del SM hace que ese tramo quede "prohibido".`,
            },
            {
              color: TEAL,
              icon: '↑',
              title: 'Zona verde — masa en exceso',
              body: `Los trabajadores que reaparecen justo por encima del nuevo SM (S/${ev.mw_new}). Muchos de ellos venían de la zona roja — fueron "empujados" al nuevo piso salarial.`,
            },
            {
              color: '#6b7280',
              icon: 'R',
              title: 'Ratio R = exceso / desaparecido',
              body: `R = ${ev.ratio.toFixed(3)} significa que por cada 100 empleos que desaparecen bajo el nuevo SM, ${Math.round(ev.ratio * 100)} reaparecen por encima. R=1 sería redistribución perfecta.`,
            },
          ].map(item => (
            <div
              key={item.title}
      ...
      <FadeSection>
        ...
        >
          <div className="font-semibold text-stone-700 text-sm">Nota: Lima Metropolitana</div>
          <p className="text-xs text-stone-500 leading-relaxed">
            Los mismos patrones se observan en EPE Lima (dataset independiente, ventanas de 6 meses,
            formalidad definida por afiliación a EsSalud). Ratios: 1.031 (2016), 0.733 (2018),
            0.885 (2022) — todos dentro de los IC bootstrap de ENAHO.
            EPE Lima usa definición de formalidad más restrictiva que ENAHO.
          </p>
        </div>
      </FadeSection>
