...
function povertyLabel(rate: number): string {
  if (rate < 0.10) return 'Baja';
  if (rate < 0.20) return 'Media-baja';
  if (rate < 0.35) return 'Media-alta';
  return 'Alta';
}
...
export default function DistritosPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><p className="text-gray-500">Cargando datos distritales...</p></div>;
  if (error) return <div className="min-h-screen flex items-center justify-center"><p className="text-red-500">Error cargando datos. <button onClick={() => window.location.reload()} className="underline">Reintentar</button></p></div>;
    ...
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="text-sm text-gray-500 mb-4">
          ...
          {' / '}
          <span className="text-gray-900 font-medium">Distritos</span>
        </nav>

        <h1 className="text-4xl font-bold text-gray-900 mb-2">Pobreza a Nivel Distrital</h1>
        <p className="text-lg text-gray-600 mb-2">
          Proxy de pobreza para {raw.length.toLocaleString()} distritos usando Nighttime Lights (NTL) satelital.
          Desagregación sub-departamental mediante dasymetric mapping.
        </p>
        <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 mb-8 inline-block">
          ⚠️ Estimaciones experimentales. NTL captura bienestar relativo, no sustituye encuestas ENAHO.
        </p>
        ...
        {filtered.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <p className="text-sm text-gray-500">Promedio (selección)</p>
              <p className="text-2xl font-bold text-gray-900">{(avgRate * 100).toFixed(1)}%</p>
            </div>
            {maxDist && (
              <div className="bg-white rounded-lg border border-red-200 p-4">
                <p className="text-sm text-gray-500">Mayor pobreza</p>
            ...
            {minDist && (
              <div className="bg-white rounded-lg border border-green-200 p-4">
                <p className="text-sm text-gray-500">Menor pobreza</p>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6 flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[160px]">
            <label className="block text-xs text-gray-500 mb-1">Buscar ubigeo / departamento</label>
          ...
          <div className="min-w-[160px]">
            ...
            >
              <option value="">Todos ({raw.length})</option>
          ...
          <div className="min-w-[180px]">
            ...
            <select
              ...
              className="w-full border border-gray-300 rounded-md text-sm px-3 py-2 focus:outline-none focus:border-blue-500"
            >
              <option value="poverty_desc">Mayor pobreza primero</option>
              <option value="poverty_asc">Menor pobreza primero</option>
              <option value="ubigeo">Ubigeo (A–Z)</option>
            </select>
          </div>

          <div className="text-sm text-gray-500 self-center">
            {filtered.length.toLocaleString()} distritos
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Año</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Pobreza Nowcast</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nivel</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Peso NTL</th>
                </tr>
              </thead>
                    ...
                    <tr key={d.district_ubigeo + i} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${povertyColor(d.poverty_rate_nowcast)}`}>
                          {povertyLabel(d.poverty_rate_nowcast)}
                        </span>
                      </td>
