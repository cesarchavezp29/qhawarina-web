  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-500">{isEn ? 'Loading inflation map...' : 'Cargando mapa de inflación...'}</p>
      </div>
    );
  }
      ...
      <div className="bg-gray-50 min-h-screen py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h1 className="text-4xl font-bold text-gray-900">
            {isEn ? 'Regional Maps — Inflation' : 'Mapas Regionales — Inflación'}
          </h1>
          <p className="text-gray-600 mt-4">{isEn ? 'No data available.' : 'No hay datos disponibles.'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-lg text-gray-600 mb-1">
          {isEn
            ? `Estimated monthly change by department · ${metadata.target_date}`
        ...
        <p className="text-sm text-gray-500 mb-4">
          ...
          {' · '}{isEn ? 'Food:' : 'Alimentos:'} <strong>{(metadata.components.food_ipc_monthly * 100).toFixed(3)}%</strong>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
              {isEn ? 'Lima (Reference)' : 'Lima (Referencia)'}
            </div>
            <div className="text-2xl font-bold text-blue-700">
              {(metadata.components.lima_headline_monthly * 100).toFixed(3)}%
            </div>
            <div className="text-xs text-gray-500 mt-1">{isEn ? 'monthly change' : 'var. mensual'} · {metadata.target_date}</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
              {isEn ? 'Highest Inflation' : 'Mayor Inflación'}
            </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
              {isEn ? 'Lowest Inflation' : 'Menor Inflación'}
            </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
              {isEn ? 'Max–Min Gap' : 'Brecha Máx–Mín'}
            </div>
            <div className="text-xl font-bold text-gray-900">{spread.toFixed(1)} pb</div>
            <div className="text-xs text-gray-500 mt-1">{isEn ? 'monthly basis points' : 'puntos básicos mensuales'}</div>
          </div>
        </div>
        ...
        {geojson && (
          <div className="bg-white rounded-lg border border-gray-200 p-6 mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {isEn ? 'Choropleth Map — Monthly Inflation by Department' : 'Mapa Coroplético — Inflación Mensual por Departamento'}
            </h2>
            <Plot
              data={[
                {
            ...
            <p className="text-xs text-gray-500 mt-3">
              {isEn
                ? `Red = higher inflation (larger food share in basket) · Green = lower inflation. Scale: 5–15 bp monthly. Data: ${metadata.target_date}.`
                : `Rojo = mayor inflación (mayor peso de alimentos en canasta) · Verde = menor inflación. Escala: 5–15 pb mensuales. Datos: ${metadata.target_date}.`}
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-200">
              <div className="text-sm font-medium text-orange-800 mb-2">
                {isEn ? 'Food & Beverages CPI' : 'IPC Alimentos y Bebidas'}
              </div>
            ...
            <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="text-sm font-medium text-blue-800 mb-2">
                {isEn ? 'Core CPI (ex-food & energy)' : 'IPC Core (sin alim. y energía)'}
              </div>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="text-sm font-medium text-gray-700 mb-2">
                {isEn ? 'Lima Metropolitan' : 'Lima Metropolitana'}
              </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <button onClick={() => setSortBy('food_share')}
                ...
                {isEn ? 'Food Share' : 'Peso Alimentos'}
              </button>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">
                    {isEn ? 'Monthly CPI' : 'IPC Mensual'}
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">
                    {isEn ? '3M Avg' : 'Promedio 3M'}
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">
                    {isEn ? '12M Approx.' : '12M Aprox.'}
                  </th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">
                    {isEn ? 'Food Share' : 'Peso Alimentos'}
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                ...
                <tr className="bg-blue-50 border-t-2 border-blue-200">
                  <td className="px-4 py-3 font-bold text-blue-900">Lima ({isEn ? 'official BCRP' : 'oficial BCRP'})</td>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-6">
          ...
          <div className="text-sm text-amber-800 space-y-2">
            <p>
              <strong>{isEn ? 'Formula:' : 'Fórmula:'}</strong>{' '}
              {isEn
                ? 'CPI_regional = α_food × CPI_food + (1 – α_food) × CPI_core. Where α_food is the food share in the departmental consumption basket according to the 2023 National Household Survey (ENAHO–INEI).'
                : 'IPC_regional = α_alim × IPC_alimentos + (1 – α_alim) × IPC_core. Donde α_alim es la participación de alimentos en la canasta de consumo departamental según la Encuesta Nacional de Hogares 2023 (ENAHO–INEI).'}
            </p>
            <p>
              <strong>{isEn ? 'Component source:' : 'Fuente de componentes:'}</strong>{' '}
              {isEn
                ? 'Food & beverages CPI (PN01383PM) and CPI excluding food and energy (PN38706PM) published monthly by BCRP.'
                : 'IPC alimentos y bebidas (PN01383PM) e IPC sin alimentos ni energía (PN38706PM) publicados mensualmente por el BCRP.'}
            </p>
            <p>
              <strong>{isEn ? 'Important limitation:' : 'Limitación importante:'}</strong>{' '}
              {isEn
                ? 'INEI only publishes official CPI for Lima Metropolitan and 25 major cities. This estimate is an econometric exercise based on heterogeneous consumption baskets — it does not replace official data. Estimated error ±3 bp monthly.'
                : 'El INEI sólo publica IPC oficial para Lima Metropolitana y 25 ciudades principales. Esta estimación es un ejercicio econométrico basado en la heterogeneidad de canastas de consumo — no reemplaza datos oficiales. El error estimado es ±3 pb mensuales.'}
            </p>
          </div>
        </div>
      </div>
